  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-4/3 w-full bg-stone-100 overflow-hidden">
        ...
        <div className="absolute left-3.5 bottom-3.5 flex flex-wrap gap-1.5 pointer-events-none">
          {item.dietaryTags.map((tag) => (
            <span
              key={tag}
              className="inline-flex items-center gap-1 bg-stone-900/90 backdrop-blur-xs border border-white/10 px-2.5 py-1 rounded-lg text-white font-sans text-[10px] font-bold tracking-wide uppercase shadow-sm"
            >
              {TagIcons[tag]}
              <span>{tag}</span>
            </span>
      ...
      <div className="p-5 flex-1 flex flex-col justify-between">
        <div>
          <div className="flex justify-between items-start gap-2 mb-2">
            <h3 className="font-extrabold text-stone-900 text-lg group-hover:text-red-600 transition-colors duration-300 leading-tight">
              {item.name}
            </h3>
            <span className="text-xl font-black text-red-600 font-sans tracking-tight shrink-0">
              {item.price}
            </span>
          </div>

          <p className="text-stone-500 font-sans text-xs md:text-sm line-clamp-2 leading-relaxed mb-4">
            {item.description}
          </p>

          {/* Quick Peek of main visual ingredients */}
          <div className="mb-4">
            <span className="text-[10px] uppercase tracking-wider font-mono font-bold text-stone-400 block mb-1.5">
              Core Stack
            </span>
            <div className="flex flex-wrap gap-1">
              ...
              {item.ingredients.length > 4 && (
                <span className="text-[10px] font-mono text-stone-400 py-0.5 px-1 font-bold">
        ...
        <div className="pt-3 border-t border-stone-100 flex items-center justify-between text-[11px] font-mono text-stone-400">
          ...
          <span className="text-red-500 font-bold group-hover:translate-x-1.5 transition-transform duration-300 flex items-center gap-1">
            Inspect Recipe <span className="font-sans">→</span>
          </span>
        </div>
      </div>
    </div>
